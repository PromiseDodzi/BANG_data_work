...
class NounParser:
    ...
    def first_parse_durationals(self, item):
        ...
        Also replaces nouns that preceed durational marking and which have contour tones, with level tones
        """
        my_list = [replacement_exceptions[i] for i in self.replacement_exceptions.keys() if len(word) >  item.index(i) + 4 and (i in item and ":" in item[item.index(i): item.index(i)+4])]
        if len(my_list) != 0:
            word_1=item[:item.index(":")-2] + "".join(my_list) + item[item.index(":"):]
        else:
            word_1=item
        ...
        for index, alphabet in enumerate(item):
            if alphabet == ":":
                ...
                if index > 0 and item[index-1] in self.replacements.keys():
                    word_2 += self.replacements[item[index-1]]
                elif index > 1 and item[index-2] in self.replacements.keys():
                    word_2 += self.replacements[item[index-2]]
            else:
                word_2 += alphabet
        
        if len(index_list) != 0 and len(my_list) !=0:
            final_word=word_1[:index_list[0]] + word_2[index_list[0]:] 
        else:
            final_word=word_2
    ...
    def second_parse_durationals(self, word):
        """
        normal parsing of durationals by doubling the vowel with the durational feature: takes care of "unresponsive" forms
        """
